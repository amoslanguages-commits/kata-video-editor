...
/// Real iOS native bridge — communicates with the Swift [IosNleEditorPlugin]
/// via MethodChannel (commands) and EventChannel (async events).
class IosNativeBridge extends NativeBridgeContract {
  ...
  Future<void> initialize() async {
    ...
    if (!result.accepted) {
      throw StateError(
        result.message ??
            result.errorCode ??
            'iOS native engine failed to initialize.',
      );
    }
  ...
  // ── Additional iOS-specific helpers ──────────────────────────────────────
  ...
  Future<NativeCommandResult> _invoke(
    ...
    try {
      ...
      return NativeCommandResult(
        ...
        accepted:  false,
        errorCode: e.code,
        message:   e.message ?? 'iOS platform channel error.',
      );
    } catch (e) {
      return NativeCommandResult(
        commandId: commandId,
        accepted:  false,
        errorCode: 'ios_bridge_error',
        message:   e.toString(),
      );
    }
  }
  ...
  NativeEvent? _decodeEvent(dynamic raw) {
    try {
      ...
      debugPrint('[IosNativeBridge] event decode error: $e  raw=$raw');
      return _errorEvent(
